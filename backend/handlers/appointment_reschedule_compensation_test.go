...
func setupAppointmentLifecycleTestDB(t *testing.T) {
	...
	if err := config.DB.AutoMigrate(&models.Card{}, &models.AppointmentCompensation{}); err != nil {
...
func TestRescheduleAppointmentClosesOldAndCreatesNew(t *testing.T) {
	...
	if err := config.DB.Table("appointments").
		Select("status, closed_reason, replaced_by_appointment_id").
		Where("id = ?", appt.ID).
		Take(&oldGot).Error; err != nil {
	...
	if err := config.DB.Table("appointments").
		Select("status, reschedule_reason").
		Where("replaces_appointment_id = ?", appt.ID).
		Take(&newGot).Error; err != nil {
	...
	if newGot.RescheduleReason != "客户主动改到下午" {

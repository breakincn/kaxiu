...
CREATE TABLE IF NOT EXISTS service_sessions (
    ...
    status ENUM('room_selecting','staff_selecting','room_locked','precheck_pending','delay_pending','serving','auto_finishing','finished','canceled') NOT NULL DEFAULT 'room_selecting' COMMENT '会话状态',
    ...
    delay_seconds INT NOT NULL DEFAULT 60 COMMENT '预结单可延迟秒数',
    duration_minutes INT NOT NULL DEFAULT 50 COMMENT '服务总时长（分钟）',
    auto_finish_delay_seconds INT NOT NULL DEFAULT 60 COMMENT '自动结单延迟秒数（服务结束后延迟自动结单）',
    auto_idle_after_seconds INT NOT NULL DEFAULT 180 COMMENT '自动结单后技师置为空闲的延迟秒数',

<template>
  <div class="min-h-screen bg-gray-50">
    <header class="bg-white px-4 py-3 flex items-center gap-3 border-b sticky top-0 z-10">
      ...
      <span class="font-medium text-gray-800">房间号牌设置</span>
    </header>
      ...
      <div v-else class="space-y-4">
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
          <div class="px-4 py-4 border-b border-gray-100">
            <div class="text-gray-800 font-medium">房间号牌编号设置</div>
          </div>
          
          <div class="px-4 py-4 space-y-4">
            <div>
              <div class="text-sm font-medium text-gray-700 mb-2">号牌前缀</div>
              <input
            ...
            <div>
              ...
              <input
                v-model.number="form.room_number_card_start_no"
                type="number"
                min="1"
                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            ...
            <div>
              ...
              <input
                v-model.number="form.room_number_card_end_no"
                type="number"
                min="1"
                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
...
const goBack = () => {
  if (window.history.length > 1) {
    router.back()
    setTimeout(() => {
      if (router.currentRoute.value.path === '/merchant/room-number-card-settings') {
...
const save = async () => {
  ...
  if (!form.value.room_number_card_start_no || form.value.room_number_card_start_no < 1) {
    alert('起始号码必须大于等于1')
    return
  }
  
  if (!form.value.room_number_card_end_no || form.value.room_number_card_end_no < 1) {
    alert('结束号码必须大于等于1')
    return
  }
  
  if (form.value.room_number_card_end_no < form.value.room_number_card_start_no) {
  ...
  try {
    await merchantApi.updateCurrentMerchantServices({
      room_number_card_prefix: form.value.room_number_card_prefix,
      room_number_card_start_no: form.value.room_number_card_start_no,
      room_number_card_end_no: form.value.room_number_card_end_no
    })
    alert('保存成功')
    await load()
  } catch (e) {
